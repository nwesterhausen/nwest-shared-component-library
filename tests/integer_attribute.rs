...
#[test]
fn test_display() {
    let mut health = IntegerAttribute::new(20);
    health -= 1;
    assert_eq!(health.current, 19);
    assert_eq!(health, 19);
    let health_display = format!("{health}");
    assert_eq!(health_display, "19 (95.00%)");
}
...
fn test_current_value() {
    let attribute = IntegerAttribute::new(100);
...
#[test]
fn test_eq() {
    let attribute1 = IntegerAttribute::new(100);
    let attribute2 = IntegerAttribute::new(100);
